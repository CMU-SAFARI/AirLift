...
//		fprintf(fp_help, "    -v INT       verbose level [%d]\n", mm_verbose);
	...
	gzFile readsFile = gzopen(argv[argc-1], "r");
	assert(readsFile);
	kseq_t *kseq_reads = kseq_init(readsFile);
	while ((mi = mm_idx_reader_read(idx_rdr, n_threads)) != 0) {
		...
		//if (!(opt.flag & MM_F_FRAG_MODE)) {
			mm_tbuf_t *tbuf = mm_tbuf_init(); // thread buffer; for multi-threading, allocate one tbuf 
			while (kseq_read(kseq_reads) >= 0) { // each kseq_read() call reads one query sequence
				int i, numOfMaps;
				mm_reg1_t *mappings;
				mappings = mm_map(mi, kseq_reads->seq.l, kseq_reads->seq.s, &numOfMaps, tbuf, &opt, 0); // get all hits for the query

				MappedReadNo=MappedReadNo+numOfMaps;
				free(mappings);
			}
	...
	if (mm_verbose >= 3) {
		...
		fprintf(stderr, "\nTotal No. of Mappings before alignment (verification): %d", MappedReadNo);
