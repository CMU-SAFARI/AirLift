...
void mm_map_frag(const mm_idx_t *mi, int n_segs, const int *qlens, const char **seqs, int *n_regs, mm_reg1_t **regs, mm_tbuf_t *b, const mm_mapopt_t *opt, const char *qname)
{
	...
	//ALSER
	for (i = 1; i < n_a; ++i){
		if (((int32_t)a[i].x - (int32_t)a[i-1].x)> ((int32_t)qlens[0])) {
			if (Seed_Num>=opt->min_cnt-1) {
				n_regs[0]++;
			}
			Seed_Num=0;
		}
		...
		//fprintf(stderr,"SD\t%d\t%s\t%d\t%c\t%d\t%d\t%d\n", Seed_Num, mi->seq[a[i].x<<1>>33].name, (int32_t)a[i].x, "+-"[a[i].x>>63], (int32_t)a[i].y, (int32_t)(a[i].y>>32&0xff), i == 0? 0 : ((int32_t)a[i].y - (int32_t)a[i-1].y) - ((int32_t)a[i].x - (int32_t)a[i-1].x));
	}
	//ALSER
	kfree(b->km, mv.a);
	kfree(b->km, a);
